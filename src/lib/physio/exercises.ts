import type { BodyRegion, ExerciseDef, ExerciseId, SessionRecord } from "./types";
...
export const EXERCISES: ExerciseDef[] = [
  {
    ...
    description:
      "The most useful knee exercise you can do at home. Stand up from a seat, sit back down with control. Camera watches knee bend and whether your torso stays tall.",
    region: "knee",
    regions: ["knee", "hip", "full"],
    ...
    cues: { down: "Sit back", up: "Stand tall", hold: "Hold the depth" },
  },
  {
    id: "shoulder_raise",
    title: "Lateral raise",
    ...
    description:
      "Raise both arms to shoulder height with a soft elbow, then lower slowly. We watch the shoulder-torso angle and whether the elbow stays long.",
    region: "shoulder",
    ...
    cues: { down: "Lower slowly", up: "Raise to shoulder", hold: "Hold height" },
  },
  {
    id: "hip_raise",
    ...
    description:
      "Lie on your back and raise one straight leg. We track the shoulder-hip-knee line so the leg stays long and the pelvis stays quiet.",
    region: "hip",
    regions: ["hip", "knee", "full"],
    difficulty: "Foundation",
    mode: "reps",
    sets: 2,
    target: 8,
    ...
    cues: { down: "Lower the leg", up: "Raise the leg", hold: "Hold height" },
  },
  {
    id: "back_stretch",
    title: "Cat-cow",
    ...
    description:
      "On all fours, you alternate a gentle dip (cow) and a rounded arch (cat). We read the shoulder-hip-knee angle to count slow cycles.",
    region: "back",
    regions: ["back", "full"],
    difficulty: "Foundation",
    mode: "reps",
    ...
    cues: { down: "Dip (cow)", up: "Round (cat)", hold: "Breathe here" },
  },
  {
    id: "glute_bridge",
    title: "Glute bridge",
    ...
    description:
      "Lie on your back, knees bent, and lift the hips until you form a shoulder-hip-knee line. We watch hip extension and stop you from over-arching.",
    region: "hip",
    regions: ["hip", "back", "knee", "full"],
    difficulty: "Build",
    mode: "reps",
    sets: 3,
    target: 12,
    ...
    cues: { down: "Lower the hips", up: "Lift the hips", hold: "Hold the line" },
  },
  {
    id: "calf_raise",
    title: "Calf raise",
    ...
    description:
      "Rise onto the balls of the feet, pause, lower the heels. We watch the heel lift and keep the knees quiet.",
    region: "ankle",
    regions: ["ankle", "knee", "full"],
    difficulty: "Foundation",
    mode: "reps",
    sets: 3,
    target: 15,
    ...
    cues: { down: "Lower the heels", up: "Rise onto toes", hold: "Hold the rise" },
  },
  {
    id: "side_leg_raise",
    ...
    description:
      "Stand on one leg and lift the other out to the side. We read the torso-hip-ankle angle so you lift from the hip, not by leaning.",
    region: "hip",
    regions: ["hip", "knee", "full"],
    difficulty: "Build",
    mode: "reps",
    ...
    cues: { down: "Lower the leg", up: "Lift to the side", hold: "Hold the lift" },
  },
  {
    id: "wall_sit",
    title: "Wall sit",
    ...
    description:
      "Slide down a wall until the knees are near 90° and hold. We watch the knee angle and credit every clean second.",
    region: "knee",
    regions: ["knee", "hip", "full"],
    difficulty: "Control",
    mode: "hold",
    ...
    cues: { down: "Bend to 90", up: "Stand to rest", hold: "Hold the sit" },
  },
];

export const EXERCISE_MAP: Record<ExerciseId, ExerciseDef> = Object.fromEntries(
  EXERCISES.map((e) => [e.id, e]),
) as Record<ExerciseId, ExerciseDef>;

export const REGIONS: { id: BodyRegion; label: string }[] = [
  { id: "full", label: "All" },
  { id: "knee", label: "Knee" },
...
const WEEKLY: ExerciseId[][] = [
  ["knee_squat", "glute_bridge", "calf_raise"],
  ["shoulder_raise", "wall_sit", "back_stretch"],
  ["hip_raise", "side_leg_raise", "glute_bridge"],
  ["knee_squat", "shoulder_raise", "calf_raise"],
  ["back_stretch", "wall_sit", "hip_raise"],
  ["glute_bridge", "side_leg_raise", "knee_squat"],
  ["calf_raise", "shoulder_raise", "back_stretch"],
];

export function getTodaysPlan(focus: BodyRegion[], date = new Date()): ExerciseDef[] {
  const ids = WEEKLY[date.getDay()] ?? WEEKLY[0];
  const list = ids.map((id) => EXERCISE_MAP[id]);
...
export function repsToday(sessions: SessionRecord[], date = new Date()): number {
  return sessions.filter((s) => isSameDay(s.date, date)).reduce((n, s) => n + s.reps, 0);
}

export function sessionsToday(sessions: SessionRecord[], date = new Date()): SessionRecord[] {
  return sessions.filter((s) => isSameDay(s.date, date));
}

export function calcStreak(sessions: SessionRecord[], date = new Date()): number {
...
export function formAverage(sessions: SessionRecord[]): number {
  if (!sessions.length) return 0;
